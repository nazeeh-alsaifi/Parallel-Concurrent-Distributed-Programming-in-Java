...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
        ...
        // Map<String, Long> nameCountMap = Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent())
        ...
        .collect(Collectors.groupingBy(Student::getFirstName, Collectors.counting())).entrySet().stream().parallel()
        .max((entry1, entry2) -> entry1.getValue() > entry2.getValue() ? 1 : -1).get().getKey();
        // nameCountMap.entrySet().stream().parallel()
        //         .max((entry1, entry2) -> entry1.getValue() > entry2.getValue() ? 1 : -1).get().getKey();
        // return Stream.of(nameCountMap.entrySet()).parallel().max((entry1,entry2) ->
