...
public class ReciprocalArraySumTest extends TestCase {
    ...
    private double parTestHelper(final int N, final boolean useManyTaskVersion, final int ntasks) {
         ...
         * accurate measurement of parallel
         * performance.
         */
    ...
    public void testParSimpleTwoMillion() {
        ...
        final String passMsg = String.format("PASSED:it was expected that the two-task parallel implementation would run at " +
                "least %fx faster and you achieved %fx speedup", minimalExpectedSpeedup, speedup);
    ...
    public void testParManyTaskTwoMillion() {
        ...
        final String errMsg = String.format("It was expected that the many-task 2m parallel implementation would run at " +
                "least %fx faster, but it only achieved %fx speedup using ncores %d", minimalExpectedSpeedup, speedup,ncores);
        assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
        final String passMsg = String.format("PASSED:it was expected that the two-task parallel implementation would run at " +
                "least %fx faster and you achieved %fx speedup", minimalExpectedSpeedup, speedup);
        System.out.println(passMsg);
    ...
    //     final String errMsg = String.format("It was expected that the many-task 200m parallel implementation would run at " +
    //             "least %fx faster, but it only achieved %fx speedup", minimalExpectedSpeedup, speedup);
    //     assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
    // }
}
